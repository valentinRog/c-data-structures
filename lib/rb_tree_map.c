...
vds_rb_tree_map vds_rb_tree_map_create( int ( *cmp )( void *, void * ),
                                        void ( *del_key )( void * ),
                                        void ( *del_value )( void * ) ) {
    vds_rb_tree_map tree = { 0 };
    tree.nil             = calloc( sizeof( vds_rb_tree_map_node ), 1 );
    tree.root            = tree.nil;
    tree.cmp             = cmp;
...
void vds_rb_tree_map_destroy( vds_rb_tree_map *tree ) {
    vds_rb_tree_map_clear( tree );
    free( tree->nil );
}

static void rotate_left( vds_rb_tree_map *tree, vds_rb_tree_map_node *x ) {
    vds_rb_tree_map_node *y = x->right;
    x->right                = y->left;
...
static void rotate_right( vds_rb_tree_map *tree, vds_rb_tree_map_node *x ) {
    vds_rb_tree_map_node *y = x->left;
    x->left                 = y->right;
...
void vds_rb_tree_map_insert( vds_rb_tree_map *tree, void *k, void *v ) {
    vds_rb_tree_map_node *p = tree->root;
    while ( p != tree->nil ) {
    ...
    vds_rb_tree_map_node *node = malloc( sizeof( vds_rb_tree_map_node ) );
    node->key                  = k;
    node->value                = v;
    node->p                    = p;
    node->left                 = tree->nil;
    ...
    while ( node != tree->root && node->p->red ) {
        if ( node->p == node->p->p->left ) {
            vds_rb_tree_map_node *y = node->p->p->right;
            if ( y->red ) {
                node->p->red    = false;
                y->red          = false;
                node->p->p->red = true;
            ...
            vds_rb_tree_map_node *y = node->p->p->left;
            if ( y->red ) {
                node->p->red    = false;
                y->red          = false;
                node->p->p->red = true;
...
static void transplant( vds_rb_tree_map      *tree,
                        vds_rb_tree_map_node *u,
                        vds_rb_tree_map_node *v ) {
    if ( !u->p ) {
        tree->root = v;
    } else if ( u == u->p->left ) {
        u->p->left = v;
...
static vds_rb_tree_map_node *tree_minimum( vds_rb_tree_map      *tree,
                                           vds_rb_tree_map_node *x ) {
    while ( x->left != tree->nil ) { x = x->left; }
    return x;
}

void vds_rb_tree_map_remove( vds_rb_tree_map *tree, void *k ) {
    vds_rb_tree_map_node *z = tree->root;
    while ( z != tree->nil ) {
    ...
    if ( !y_original_color ) {
        while ( x != tree->root && !x->red ) {
            if ( x == x->p->left ) {
                vds_rb_tree_map_node *w = x->p->right;
                if ( w->red ) {
                    w->red    = false;
                    x->p->red = true;
                ...
                vds_rb_tree_map_node *w = x->p->left;
                if ( w->red ) {
                    w->red    = false;
                    x->p->red = true;
...
void vds_rb_tree_map_clear( vds_rb_tree_map *tree ) {
    while ( tree->size ) { vds_rb_tree_map_remove( tree, tree->root->key ); }
}
